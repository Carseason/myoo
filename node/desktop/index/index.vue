    ...
    <div class="body">
        <template v-for="( item , i) in Data">
            <div class="row-box" :key="i" ref="BoxScroll">
                ...
                <div v-if=" item.Code != ''" class="box-code" v-html="item.Code"></div>
            </div>
        </template>
        <div v-if="Count > 0">
...
export default {
    ...
    methods: {
        Scrollshow: function (e) {
            this.$refs.BoxScroll[e].scrollIntoView({   //滑动效果
                behavior: "smooth"
            });
        ...
        '$store.state.ScrollTop': function () {
            ...
            if (this.$store.state.ScrollStatus) {
                ...
                for (let i = 0; i < this.Count; i++) {
                    if (this.$refs.BoxScroll[i].offsetTop <= this.$store.state.ScrollTop) {
                        this.scrollshow = i;
                    } else {
                        break;
                    }
